...
import { getMAScount } from "./part2/solution.ts";
...
Deno.test("it should get good result with full exercice input", () => {
  ...
  const inputParsed = parseInput(input);
  assertEquals(getXMAScount(inputParsed), 2551);
});

// SECOND EXERCICE
Deno.test("it should get good result with sample input", () => {
  const input = Deno.readTextFileSync("DAY04/part2/example-input.text");
  const inputParsed = parseInput(input);
  assertEquals(getMAScount(inputParsed), 9);
});

Deno.test("it should get good result with full exercice input", () => {
  const input = Deno.readTextFileSync("DAY04/input.text");
  const inputParsed = parseInput(input);
  assertEquals(getMAScount(inputParsed), 1985);
});
